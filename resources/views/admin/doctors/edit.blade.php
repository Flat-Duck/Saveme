...
<div class="row">
    <div class="col-xs-12">
        <div class="box box-primary">
            ...
            <form role="form" method="POST" enctype="multipart/form-data" action="{{ route('admin.doctors.update', ['doctor' => $doctor->id]) }}">
                @csrf
                @method('PUT')
                        ...
                        >
                            @foreach ($clinks as $clink)
                                <option value="{{ $clink->id }}"
                                    {{ old('clink_id', $doctor->clink_id) == $clink->id ? 'selected' : '' }}
                                >
                                    {{ $clink->name }}
                                </option>
                            @endforeach
